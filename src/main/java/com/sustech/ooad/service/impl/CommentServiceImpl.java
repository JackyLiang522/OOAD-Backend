...
public class CommentServiceImpl implements CommentService {
    ...
    public Comment addComment(Chapter chapter, String contents, int id, String nickname, int user_id, Date date) {
        Comment comment = new Comment(chapter, contents, id, nickname, user_id, date);
    ...
    public Comment updateComment(int user_id, long id, String contents) {
        Comment c = commentRepository.findById(id).orElse(null);
        if (c != null && c.getUser_id() == user_id){
            Comment newComment = new Comment(c.getChapter(), contents, (int)id, c.getNickname(), c.getUser_id(), c.getCreateTime());
            commentRepository.delete(c);
